...
export class MyPlane extends CGFobject {
	constructor(scene,isTiledPlane, nrDivs, minS, maxS, minT, maxT,) {
		super(scene);
		this.isTiledPlane = isTiledPlane
	...
	initBuffers() {
		...
		for (var j = 0; j <= this.nrDivs; j++) {
			...
			for (var i = 0; i <= this.nrDivs; i++) {
				...
				if(this.isTiledPlane){
					const repeatFactor = 10;//10 tiles of 40 units per row, 10x10
					this.texCoords.push(
						(this.minS + i * this.q) * repeatFactor,
						(this.minT + j * this.w) * repeatFactor
					);
				}
